...
class Personaje {
  ...
  factory Personaje.fromJson(Map<String, dynamic> json) => Personaje(
    id: json["id"] ?? 0,
    age: json["age"] ?? -1,
    //birthdate: DateTime.parse(json["birthdate"] ?? '2000-05-03'),
    description: json["description"] ?? 'Sin información',
    //firstAppearanceEpId: json["first_appearance_ep_id"],
    //firstAppearanceShId: json["first_appearance_sh_id"],
    gender: json["gender"] ?? 'Sin información',
    name: json["name"] ?? 'Sin información',
    occupation: json["occupation"] ?? 'Sin información',
    phrases: List<String>.from(
      json["phrases"].map((x) => x) ?? 'Sin información',
    ),
    portraitPath: json["portrait_path"],
    status: json["status"] ?? 'Sin información',
